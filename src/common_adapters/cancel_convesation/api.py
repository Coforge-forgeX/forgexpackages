    ...
    async def mark_cancelled(self, key: str, req: CancelRequest) -> None:
        ...
        if cache is not None:
            try:
                ...
                # Redis misconfigured/unreachable at runtime.
                # Keep logs explicit because multi-worker STOP depends on this.
                logger.warning(f"[CANCEL_STORE] Redis write failed; falling back to local memory. key={key} err={e}")
                self._disable_cache()
    ...
    async def is_cancelled(self, key: str) -> bool:
        ...
        if cache is not None:
            try:
                ...
                result = await cache.retrieve_data(key)
                # If Redis is misconfigured (host empty) or not reachable, disable cache
                # to avoid repeated long polls.
                if result and result.get("status") != "success":
                ...
                if cache is not None and result and result.get("status") == "success":
                    ...
                    if cached:
                        ...
                        if ts and (time.time() - ts) <= _CANCEL_TTL_SECONDS:
                            ...
                            logger.info(
                                f"[CANCEL_CHECK] Found in Redis but EXPIRED (age: {time.time() - ts:.1f}s, TTL: {_CANCEL_TTL_SECONDS}s)"
                            )
                    else:
            ...
            logger.warning(
                "[CANCEL_CHECK] Redis cache unavailable - using local memory only (multi-worker cancellation may be delayed if requests hit different workers)"
            )
