...
class ReactGraphState(MessagesState):
    ...
    user_id: str
    conversation_id: str
    jira_tool_llm: langchain_core.runnables.base.RunnableBinding


async def initialize_state_history(state: ReactGraphState,session_manager , msgs)-> ReactGraphState:
...
async def jira_assistant(state: ReactGraphState) -> ReactGraphState:

    llm_with_tools = state.get('jira_tool_llm', langchain_core.runnables.base.RunnableBinding)
    
    response = await llm_with_tools.ainvoke(state.get("messages",list))
    
    # only for debugging.
    # logger.info(f"jira_assistant_response: {response}")
